...
3. Tampermonkey 儀表板 → 點進 **Threads Scriber (Auto, crawl-the-threads)** 進編輯器（這個 editor 分頁必須一直開著，helper 會用標題自動找）
4. venv 啟動：`.\.venv\Scripts\Activate.ps1`
5. `.env` 裡 `CHROME_WS_PATH` 已設好
...
| 出錯訊號 | 原因 | 怎麼修 |
| --- | --- | --- |
| `push_userscript.py` exit 2「no Tampermonkey editor tab」 | 編輯器分頁沒開 | 開 Tampermonkey dashboard，點進「Threads Scriber (Auto, crawl-the-threads)」 |
| `push_userscript.py` exit 2「CHROME_WS_PATH not set」 | `.env` 沒設或被清空 | 補回 `CHROME_WS_PATH`（見 README 第 0.B 節） |
...
- 純文件修改（README、註解）
- 純 config 修改（`classify_config.json`、`.env`）
