...
> **最新更新（2026-09-18）**：userscript 0.6.1。修正 Threads 改版後「逐篇取消儲存」找不到貼文選單的問題：貼文「⋯」圖示的 `aria-label` 被 Threads 移除，舊版會誤點到回覆排序選單而全部失敗；現在改以 `aria-haspopup="menu"` 辨識貼文選單。0.6.0 的桌面版 host adapter 與 0.5.5 的內文黏連修正一併包含。請在 Tampermonkey 重新安裝 userscript。完整紀錄見 [RELEASE_NOTES.md](RELEASE_NOTES.md)。
